...
//	// =========================================================
//	// 💡 [수정] 매 프레임 발밑을 확인해서 높이를 실시간으로 맞춥니다!
//	// =========================================================
//	private void UpdateFloatingPosition()
//	{
//		// 1. 매 프레임 내 발밑에 어떤 바닥이 있는지 실시간으로 레이캐스트를 쏩니다.
//		Vector2 gravityDir = (_currentDirection == eGravityDirection.Normal) ? Vector2.down : Vector2.up;
...
//		{
//			if (h.collider != null && h.collider != _col && !h.collider.isTrigger)
//			{
//				// 2. 발밑에 바닥(새로운 바닥이든 기존 바닥이든)을 찾았다면?
//				_surfaceCollider = h.collider;
//				float halfHeight = _col.bounds.extents.y;

//				// 새로운 목표 높이를 갱신합니다.
//				_floatCenter = h.point - gravityDir * (halfHeight + surfaceGap);
//				_surfaceLocalOffset = _floatCenter - (Vector2)_surfaceCollider.transform.position;
//				foundSurface = true;
//				break; // 가장 먼저 닿은 바닥 하나만 찾으면 끝!
//			}
//		}

//		// 3. 만약 낭떠러지로 밀려서 발밑에 바닥이 없어졌다면?!
//		if (!foundSurface)
//		{
//			_surfaceCollider = null;
//			// 바닥을 찾을 때까지 중력 방향(아래)으로 계속 떨어지도록 목표점을 저 아래로 세팅합니다.
//			_floatCenter = _rb.position + gravityDir * 5f;
//		}

//		// 4. 현재 큐브 위치와 목표 위치(floatCenter)가 차이가 난다면?
//		// (낭떠러지로 떨어져야 하거나, 계단처럼 높은 곳으로 밀려 올라갔을 때)
//		if (Mathf.Abs(_rb.position.y - _floatCenter.y) > 0.15f)
//		{
//			// 둥둥거리는 파도타기(Waving)를 멈추고 목표 지점까지 부드럽게 추락/상승하도록 상태를 바꿉니다.
//			_floatingState = FloatingState.Ascending;
//		}
//	}

//	// =========================================================
//	// 💡 [수정] 목표 지점까지 이동(추락/상승)하는 로직
//	// =========================================================
//	private void HandleAscending()
//	{
//		Vector2 pos = _rb.position;
//		// 허공에서 떨어질 때도 부유(Ascend) 속도에 맞춰서 깃털처럼 스무스하게 떨어집니다.
//		float newY = Mathf.MoveTowards(pos.y, _floatCenter.y, ascendSpeed * Time.fixedDeltaTime);
//		_rb.MovePosition(new Vector2(pos.x, newY));

//		// 목표 지점에 거의 도달했고, 내 발밑에 진짜 바닥이 있을 때만 다시 파도타기(둥둥) 시작!
//		if (Mathf.Abs(newY - _floatCenter.y) < 0.01f && _surfaceCollider != null)
//		{
